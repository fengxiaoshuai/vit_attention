...
class TestFusedVitAttentionOp(OpTest):
    ...
    def setUp(self):
        ...
        # W = QKV
        w = hidden_size * 3
        self.Input = np.random.random((self.batch_size, h, w)).astype("float32") - 0.5
        # reshape
        reshape_input = np.reshape(self.Input, (self.batch_size, self.seq_len,3, self.head_number, self.size_per_head))
        # transpose
        ...
        reshape_qkv = np.reshape(transpose_qkv, (self.batch_size, h, hidden_size))
        self.inputs = {
            "Input": self.Input
        }
        self.attrs = {
