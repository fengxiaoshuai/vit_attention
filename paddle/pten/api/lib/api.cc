...
PADDLE_API Tensor add(const Tensor& x, const Tensor& y) {
  ...
  auto out_meta = pten::ElementwiseInferMeta(dense_x->meta(), dense_y->meta(), -1);
...
PADDLE_API Tensor flatten(const Tensor& x, int start_axis, int stop_axis) {
  ...
  auto out_meta = pten::FlattenInferMeta(dense_x->meta(), start_axis, stop_axis);
...
PADDLE_API Tensor full(const ScalarArray& shape, const Scalar& value, DataType dtype, Backend place, DataLayout layout) {
  ...
  (*kernel_fn)(*dev_ctx, pten::ScalarArray(shape), pten::Scalar(value), dense_out.get());

  return out;
}

PADDLE_API Tensor full_like(const Tensor& x, const Scalar& value, DataType dtype, Backend place, DataLayout layout) {
...
PADDLE_API Tensor matmul(const Tensor& x, const Tensor& y, bool transpose_x, bool transpose_y) {
  ...
  (*kernel_fn)(*dev_ctx, *dense_x, *dense_y, transpose_x, transpose_y, dense_out.get());

  return out;
}

PADDLE_API Tensor mean(const Tensor& x, const std::vector<int64_t>& axis, bool keep_dim) {
...
PADDLE_API Tensor ones_like(const Tensor& x, DataType dtype, Backend place, DataLayout layout) {
...
PADDLE_API Tensor scale(const Tensor& x, const Scalar& scale, float bias, bool bias_after_scale) {
  ...
  (*kernel_fn)(*dev_ctx, *dense_x, pten::Scalar(scale), bias, bias_after_scale, dense_out.get());
...
PADDLE_API Tensor sum(const Tensor& x, const std::vector<int64_t>& axis, DataType dtype, bool keep_dim) {
  ...
  (*kernel_fn)(*dev_ctx, *dense_x, axis, keep_dim, false, DataType::UNDEFINED, dense_out.get());

  return out;
}

PADDLE_API Tensor zeros_like(const Tensor& x, DataType dtype, Backend place, DataLayout layout) {
