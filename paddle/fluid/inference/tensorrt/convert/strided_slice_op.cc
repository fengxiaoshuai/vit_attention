...
class StridedSliceOpConverter : public OpConverter {
 public:
  void operator()(const framework::proto::OpDesc& op,
    ...
    for(int i=0; i<start.nbDims; i++) {start.d[i] = 0;}
    for(int i=0; i<axes.size(); i++) {start.d[axes[i]] = starts[i];}


    nvinfer1::Dims stride;
    stride.nbDims = input_dims.nbDims;
    for(int i=0; i<stride.nbDims; i++) {stride.d[i] = 1;}
    for(int i=0; i<axes.size(); i++) {stride.d[axes[i]] = strides[i];}

    nvinfer1::Dims size;
    size.nbDims = input_dims.nbDims;
    for(int i=0; i<size.nbDims; i++) {size.d[i] = 1;}

    auto output_name = op_desc.Output("Out")[0];

    auto create_weights = [&](const std::vector<int>& data, const std::string& type) -> int* 
    {
    ...
    for(int i=0; i<axes.size(); i++) {const_weight[axes[i]] = strides[i];}

    int* weight_data = create_weights(const_weight, "size");
    
    TensorRTEngine::Weight weight{nvinfer1::DataType::kINT32, static_cast<void*>(weight_data),static_cast<size_t>(input_dims.nbDims)};

    auto const_layer = TRT_ENGINE_ADD_LAYER(engine_, Constant, nvinfer1::Dims{1,{input_dims.nbDims}}, weight.get());

    auto shape_layer = TRT_ENGINE_ADD_LAYER(engine_, Shape, *input);

    auto size_layer = TRT_ENGINE_ADD_LAYER(engine_, ElementWise, *shape_layer->getOutput(0), *const_layer->getOutput(0), 
                          nvinfer1::ElementWiseOperation::kDIV);

    auto* layer = TRT_ENGINE_ADD_LAYER(engine_, Slice, *input, start, size, stride);
