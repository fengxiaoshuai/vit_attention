...
SlicePluginDynamic::SlicePluginDynamic(std::vector<int> starts,
                                       std::vector<int> ends,
                                       std::vector<int> axes, int decrease_axis, bool with_fp16)
...
size_t SlicePluginDynamic::getSerializationSize() const TRT_NOEXCEPT {
  size_t size = SerializedSize(starts_) + SerializedSize(ends_) +
                SerializedSize(axes_) + SerializedSize(decrease_axis_) + SerializedSize(with_fp16_);
...
nvinfer1::DimsExprs SlicePluginDynamic::getOutputDimensions(
  ...
  if(decrease_axis_!=-1)
  {
      ...
      for (size_t i = 0; i < in_dims.nbDims; i++)
      {
          ...
          res.d[j++] = expr_builder.operation(nvinfer1::DimensionOperation::kMAX, *expr_builder.constant(0), *in_dims.d[i]);
      };
      return res;
  }
  return ret;
